# 内容待补充清单 — IE Pro 400 GlobalStandard 开发者资料

**用途**：本文件面向内部团队，汇总当前仓库中所有 `[待补充]/[TBD]` 占位符，按主题归类为可执行的待办事项，方便分工认领。不属于对外发布的9份文档之一。

**当前状态**：`docs/` 正文已无 `[待补充]`/`[TBD]` 占位符；下方清单仅保留**仍需补充**的未完成项（共 1 项）。
...
- [x] CAN 接口：数量、协议标准（CAN2.0A/B等）、波特率范围、端接电阻是否内置/可拨码 → 端接电阻为现场外接方案，非内置/拨码
- [x] 数字量 I/O：DI/DO 数量、电气类型（干接点/湿接点/继电器/晶体管）、电压与负载范围 → X1 无源输入、Y1 无源输出（干接点）
- [x] 4G 规格：模组型号、制式（LTE FDD/TDD）、**支持频段需覆盖全部目标销售区域**、SIM 卡槽数量与类型、天线接口类型
- [x] 其他接口：USB、SD/TF 卡槽、Console 调试口、指示灯含义对照表、按键功能（含 Reset）
...
- [x] GitHub 仓库地址、访问方式（公开 / 需申请账号）→ https://github.com/anylink-dev/IEPro.git（公开仓库）
...
- [x] 是否存在子型号/区域版本差异 → IE Pro 400 GlobalStandard 全球通版
...
*最后更新：2026-07-31。未完成项请由对应负责人补充后勾选。*
